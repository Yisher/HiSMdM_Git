...
class UNet_Dataset_NoNorm(Dataset):
    def __init__(self, dir_input, dir_GT):
        super(UNet_Dataset_NoNorm, self).__init__()
        self.dir_input = dir_input
        self.dir_GT = dir_GT
        self.files = os.listdir(self.dir_input)
        self.trans = transforms.Compose((
    ...
    def __getitem__(self, item):
        ...
        if file_name.startswith('frame_'):
            input_ = Image.open(os.path.join(self.dir_input, file_name)).convert('F')

            frame_file_name = file_name.replace('frame_', 'frame0').zfill(10)
            GT = Image.open(os.path.join(self.dir_GT, frame_file_name)).convert('F')
        else:
            raise FileNotFoundError(f"File {file_name} not found in the noisy folder.")
        
        return self.trans(input_), self.trans(GT)        
